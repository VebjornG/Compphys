...
// General solution with 9(n-1) flops
double gen_solv(int n, double* v) {
    ...
    cout << setprecision(25) << setw(15) << "The time used for vector addition general case = " << runtime_gen << endl;
    return runtime_gen;
};
// End gen_sol
...
double Special_solv(int n, double* v_) {
    ...
    cout << setiosflags(ios::showpoint | ios::uppercase);
    cout << setprecision(25) << setw(15) << "The time used for vector addition special case = " << runtime_special << endl;
    return runtime_special;
};

/*double epsilon(double *u, double *v) {
    return double log10(fabs((v - u)/u)
